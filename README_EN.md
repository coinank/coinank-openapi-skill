...
| **Auth** | Direct mode: `apikey: <your_api_key>`; x402 mode: payment header generated by the payment flow |
| **Method** | All `GET` |
...
The endpoint examples below default to **direct API-key mode**. In x402 mode, the URL and request parameters stay the same; the agent simply replays the same request with the required payment header after payment succeeds.
